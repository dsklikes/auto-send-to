...
export class Widget {
    ...
    static async getWidgets(subredditName, metadata) {
        const client = Devvit.getApiClient(Devvit.Types.RedditAPI.Widgets);
        ...
        return (response.layout.sidebar?.order?.map((widgetId) => {
            const widgetData = widgetsMap[widgetId];
            switch (widgetData.kind) {
                case 'image':
                    return new ImageWidget(widgetData, subredditName, metadata);
                case 'calendar':
                    return new CalendarWidget(widgetData, subredditName, metadata);
                case 'textarea':
                    return new TextAreaWidget(widgetData, subredditName, metadata);
                case 'button':
                    return new ButtonWidget(widgetData, subredditName, metadata);
                case 'community-list':
                    return new CommunityListWidget(widgetData, subredditName, metadata);
                case 'post-flair':
                    return new PostFlairWidget(widgetData, subredditName, metadata);
                case 'custom':
                    return new CustomWidget(widgetData, subredditName, metadata);
                default:
